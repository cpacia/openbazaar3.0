...
func TestOrderProcessor_processOrderFulfillmentMessage(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expectedError: nil,
			expectedEvent: &events.OrderFulfillment{
				OrderID: orderID,
		...
		},
		{
			...
			setup: func(order *models.Order) error {
				return order.PutMessage(&npb.OrderMessage{
					Signature:   []byte("abc"),
					Message:     mustBuildAny(fulfillmentMsg),
					MessageType: npb.OrderMessage_ORDER_FULFILLMENT,
				})
			},
			expectedError: nil,
	...
	for i, test := range tests {
		...
		err := op.db.Update(func(tx database.Tx) error {
			event, err := op.processOrderFulfillmentMessage(tx, order, remotePeer, orderMsg)
